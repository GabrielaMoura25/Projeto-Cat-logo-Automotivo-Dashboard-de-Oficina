...
describe("Product Service", () => {
    it("cria produto e retorna id", async () => {
        ...
        const id = await productService.criarProduto({ nome: "Teste", categoria: "Teste" });
        expect(id).toBe(123);
    });

    it("propaga erro do model ao criar produto", async () => {
        Product.create.mockRejectedValue(new Error("DB error"));
        await expect(productService.criarProduto({ nome: "Teste", categoria: "Teste" })).rejects.toThrow("DB error");
    });
    ...
    it("propaga erro do model ao buscar produtos", async () => {
        ...
        await expect(productService.buscarProdutos("Filtro")).rejects.toThrow("DB error");
    });

    it("propaga erro do model ao atualizar produto", async () => {
        Product.update.mockRejectedValue(new Error("DB error"));
        await expect(productService.atualizarProduto(1, { nome: "Teste" })).rejects.toThrow("DB error");
    });
