...
async function gerarDescricao(nome, categoria) {
    ...
    try {
        ...
        if (Array.isArray(response.outputs) && response.outputs[0] && response.outputs[0].content) {
 ...
 */
async function gerarDescricaoComRetry(nome, categoria, tentativas = 3, timeoutMs = 7000) {
